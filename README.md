...
| [Snoopy](https://github.com/Sweatzer/Lab-WriteUps/blob/main/Pentest-Writeups/HTB-Machines/snoopy/snoopy.md) | Hard / Linux | `....//` path-traversal file read in `/download` → BIND `rndc` TSIG key disclosure → authenticated DNS zone takeover (`nsupdate`) → impersonate offline mail server to intercept Mattermost password reset (`sbrown`) → `/server_provision` + SSH honeypot captures `cbrown` creds → `sudo git apply` symlink traversal (CVE-2023-23946) → `sudo clamscan` DMG XXE (CVE-2023-20052) leaks root SSH key. Full client-style report: 7 findings, CVSS severities, remediation | PTES / OWASP, MITRE CWE-22 / 200 / 522 / 611 / 640, CVE-2023-23946 / CVE-2023-20052 |
